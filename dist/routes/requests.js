...
router.get('/collection/:collectionId', async (req, res) => {
    try {
        ...
        const collectionCheck = await pool.query('SELECT id FROM collections WHERE id = $1 AND workspace_id = $2', [collectionId, workspaceId]);
        if (collectionCheck.rows.length === 0) {
            return res.status(404).json({ error: 'Collection not found' });
        }
        const result = await pool.query(`SELECT r.*, u.name as created_by_name
       FROM requests r
       LEFT JOIN users u ON r.created_by = u.id
       WHERE r.collection_id = $1
       ORDER BY r.created_at ASC`, [collectionId]);
        res.status(200).json({
...
router.get('/:id', async (req, res) => {
    try {
       ...
       WHERE r.id = $1 AND c.workspace_id = $2`, [id, workspaceId]);
...
router.post('/', async (req, res) => {
    try {
        ...
        // Verify collection belongs to user's workspace
        const collectionCheck = await pool.query('SELECT id FROM collections WHERE id = $1 AND workspace_id = $2', [collection_id, workspaceId]);
        if (collectionCheck.rows.length === 0) {
            return res.status(404).json({ error: 'Collection not found' });
        }
        const result = await pool.query(`INSERT INTO requests (name, method, url, headers, body, params, collection_id, created_by)
